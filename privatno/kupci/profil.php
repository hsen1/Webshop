...
if(isset($_POST["sifra"])){
	...
	if(strlen(trim($_POST["lozinka"]))>0){
		//mijenja se lozinka
		$izraz=$veza->prepare("update kupac set ime=:ime, prezime=:prezime, email=:email, lozinka=md5(:lozinka) where sifra=:sifra");
	}else{
		//ne mjenja se lozinka
		unset($_POST["lozinka"]);
		$izraz=$veza->prepare("update kupac set ime=:ime, prezime=:prezime, email=:email where sifra=:sifra");
	}
